...
/**
 * @author abarendt
 * This class tests the scientific name field
 */
public class TextFieldCombinations_ScientificName extends
	...
	private String extractedName_Sci = "";
	...
	public TextFieldCombinations_ScientificName() {
	...
	protected void setUp() throws Exception {
		...
		birdNameLabelText = getActivity().getResources().getString(idBirdNameLabelText);
		birdNameLabelText = birdNameLabelText.substring(0, (birdNameLabelText.length()-6));
		notesLabelText = getActivity().getResources().getString(idNotesLabelText);
		notesLabelText = notesLabelText.substring(0, notesLabelText.length()-1);
		sciNameLabelText=getActivity().getResources().getString(idSciNameLabelText);
		sciNameLabelText = sciNameLabelText.substring(0,sciNameLabelText.length()-6);
		
		
	}
	/**
	 * When I have information in the Scientific Name field and the Notes field
	 * And I click the submit button,
	 * An alert box will pop up asking me if I want to submit the information with an empty Bird Name field.
	 * If I click No,
	 * I will be returned to the main screen
	 * And the text I have already entered will still be present.
	 * If I click Yes,
	 * I will see a message that my information has been submitted.
	 */
	public void testSciNotesOnly(){ solo.assertCurrentActivity("testSciCommonOnly", BirdFormActivity.class);
		...
		assertFalse(solo.searchText(sciNameLabelText));
	...
	/**
	 *  When I have information in the Scientific Name field and the Bird Name field
	 * And I click the submit button,
	 * An alert box will pop up asking me if I want to submit the information with an empty Note field.
	 * If I click No,
	 * I will be returned to the main screen
	 * And the text I have already entered will still be present.
	 * If I click Yes,
	 * I will see a message that my information has been submitted.
	 */
	public void testSciCommonOnly(){ 
		solo.assertCurrentActivity("testSciCommonOnly", BirdFormActivity.class);
	...
	/**
	 * Test that the actual information that is entered into the Scientific Name field
	 */
	public void testActualScientificNameReturned() {
		solo.assertCurrentActivity("testScientificName", BirdFormActivity.class);
		solo.enterText(scientificName, SCIENTIFIC_NAME);
		extractedName_Sci = scientificName.getText().toString();
		assertEquals(SCIENTIFIC_NAME, extractedName_Sci);
		solo.finishOpenedActivities();
	}

	/**
	 * Tests that the text field is long enough for a very long trinomial scientific name
	 * by verifying that the length of the string that was put in the field is the same
	 * length of the text coming out of the field.
	 */
	public void testIsEqualLength() {
		solo.assertCurrentActivity("testIsEqualLength", BirdFormActivity.class);
		solo.enterText(scientificName, SCIENTIFIC_NAME);
		assertEquals(SCIENTIFIC_NAME.length(), scientificName.getText().toString().length());
		solo.finishOpenedActivities();
	}

	/**
	 *When I have information in the Scientific Name field only
	 * And I click the submit button,
	 * An alert box will pop up asking me if I want to submit the information with empty Note and Bird Name fields.
	 * If I click No,
	 * I will be returned to the main screen
	 * And the text I have already entered will still be present.
	 * If I click Yes,
	 * I will see a message that my information has been submitted.
	 */
	public void testScientificNameOnly() {
		solo.assertCurrentActivity("testScientificNameOnly",
